...
import {events, payment} from "@/lib/domain";
    ...
    return (
        <main className="flex min-h-screen flex-col max-w-screen-lg m-auto gap-2">
            ...
            <section className="flex flex-col items-center justify-center mt-6 text-center">
                <span className="font-bold">A jegyeket a megadott email címre küldjük el.</span>
                <span className="mt-2">Ellenőrizze a Spam és a Levélszemét mappákat is. Előfordulhat, hogy a jegyeket tartalmazó e-mail oda érkezik.</span>
                <span className="mt-2">(Megrendelési azonosító: {paymentResult.orderId})</span>
                <span className="mb-4">(Tranzakciós azonosító: {paymentResult.transactionId})</span>
            </section>
        </main>
    );
}
